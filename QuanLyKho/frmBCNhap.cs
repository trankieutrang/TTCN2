...
namespace QuanLyKho
{
    public partial class frmBCNhap : Form
    {
        ...
        private void frmBCNhap_Load(object sender, EventArgs e)
        {
            ...
            loadDataToGridView();
            dataGridView_BCNhap.DataSource = null;
        }
        DataTable tableHT;
        private void loadDataToGridView()
        {
            string sql;
            sql = "	select a.MaNhomHang,b.TenHang,d.NgayLap,c.SoLuong,c.DonGia, c.TongCong from NhomHang a join HangHoa b on a.MaNhomHang=b.MaNhomHang join ChiTietPhieuNK c on b.MaHang=c.MaHang join PhieuNK d on d.MaPhieuNK=c.MaPhieuNK";
            tableHT = Class.Functions.GetDataToTable(sql);
            dataGridView_BCNhap.DataSource = tableHT;
            dataGridView_BCNhap.Columns[0].HeaderText = "Mã NH";
            dataGridView_BCNhap.Columns[1].HeaderText = "Tên Hàng";
            dataGridView_BCNhap.Columns[2].HeaderText = "Ngày Lập";
            dataGridView_BCNhap.Columns[3].HeaderText = "Số Lượng";
            dataGridView_BCNhap.Columns[4].HeaderText = "Đơn Giá";
            dataGridView_BCNhap.Columns[5].HeaderText = "Thành tiền";
            // Không cho phép thêm mới dữ liệu trực tiếp trên lưới
            dataGridView_BCNhap.AllowUserToAddRows = false;
            // Không cho phép sửa dữ liệu trực tiếp trên lưới
            dataGridView_BCNhap.EditMode = DataGridViewEditMode.EditProgrammatically;
        }

        private void dataGridView_BCNhap_Click(object sender, EventArgs e)
        {
            txtMaNH.Text = dataGridView_BCNhap.CurrentRow.Cells["MaNhomHang"].Value.ToString();
            txtTenHang.Text = dataGridView_BCNhap.CurrentRow.Cells["TenHang"].Value.ToString();
            mskNgayLap.Text = dataGridView_BCNhap.CurrentRow.Cells["NgayLap"].Value.ToString();
            txtDonGia.Text = dataGridView_BCNhap.CurrentRow.Cells["DonGia"].Value.ToString();
            txtSoLuong.Text = dataGridView_BCNhap.CurrentRow.Cells["SoLuong"].Value.ToString();
            txtthanhtien.Text = dataGridView_BCNhap.CurrentRow.Cells["TongCong"].Value.ToString();
        }
        ...
        private void btnTimKiem_Click(object sender, EventArgs e)
        {
            ...
            sql = "Select a.MaNhomHang,b.TenHang,c.SoLuong,c.DonGia,d.NgayLap, c.TongCong from NhomHang a join HangHoa b on a.MaNhomHang=b.MaNhomHang join ChiTietPhieuNK c on b.MaHang=c.MaHang join PhieuNK d on d.MaPhieuNK=c.MaPhieuNK WHERE 1=1";
            if (cboNhomHang.Text != "")
            ...
            else
            {
                ...
                if (cboNhomHang.Text != "")
                {
                    tong = Convert.ToDouble(Class.Functions.GetFieldValues("select sum(c.TongCong) from NhomHang a join HangHoa b on a.MaNhomHang=b.MaNhomHang join ChiTietPhieuNK c on b.MaHang=c.MaHang join PhieuNK d on d.MaPhieuNK=c.MaPhieuNK where a.TenNhomHang='" + cboNhomHang.Text + "'"));
                    txtTongTien.Text = tong.ToString();
                    lblBangChu.Text = "Bằng chữ: " + Class.Functions.ChuyenSoSangChu(tong.ToString());
                }
                if ((cboNhomHang.Text != "") && (cboThang.Text != ""))
                {
                    tong = Convert.ToDouble(Class.Functions.GetFieldValues("select sum(c.TongCong) from NhomHang a join HangHoa b on a.MaNhomHang=b.MaNhomHang join ChiTietPhieuNK c on b.MaHang=c.MaHang " +
                        "join PhieuNK d on d.MaPhieuNK=c.MaPhieuNK where a.TenNhomHang='" + cboNhomHang.Text + "'and MONTH(d.NgayLap)='"+cboThang.Text+"' "));
                    txtTongTien.Text = tong.ToString();
                    lblBangChu.Text = "Bằng chữ: " + Class.Functions.ChuyenSoSangChu(tong.ToString());
                }
                ...
                {
                    tong = Convert.ToDouble(Class.Functions.GetFieldValues("select sum(c.TongCong) from NhomHang a join HangHoa b on a.MaNhomHang=b.MaNhomHang join ChiTietPhieuNK c on b.MaHang=c.MaHang " +
                        "join PhieuNK d on d.MaPhieuNK=c.MaPhieuNK  where TenNhomHang='" + cboNhomHang.Text + "'and DATEPART(qq,(d.NgayLap))='" + cboQuy.Text + "'"));
                    txtTongTien.Text = tong.ToString();
                    lblBangChu.Text = "Bằng chữ: " + Class.Functions.ChuyenSoSangChu(tong.ToString());
                }
                ...
                {
                    tong = Convert.ToDouble(Class.Functions.GetFieldValues("select sum(c.TongCong) from NhomHang a join HangHoa b on a.MaNhomHang=b.MaNhomHang join ChiTietPhieuNK c on b.MaHang=c.MaHang " +
                        "join PhieuNK d on d.MaPhieuNK=c.MaPhieuNK  where TenNhomHang='" + cboNhomHang.Text + "'and DATEPART(yyyy,(d.NgayLap))='" + txtNam.Text + "'"));
                    txtTongTien.Text = tong.ToString();
                    lblBangChu.Text = "Bằng chữ: " + Class.Functions.ChuyenSoSangChu(tong.ToString());
                }
                ...
                {
                    tong = Convert.ToDouble(Class.Functions.GetFieldValues("select sum(c.TongCong)from NhomHang a join HangHoa b on a.MaNhomHang=b.MaNhomHang join ChiTietPhieuNK c on b.MaHang=c.MaHang " +
                        "join PhieuNK d on d.MaPhieuNK=c.MaPhieuNK where DATEPART(mm,(d.NgayLap)) = '" + cboThang.Text + "'" +
                ...
                {
                    tong = Convert.ToDouble(Class.Functions.GetFieldValues("select sum(c.TongCong)from NhomHang a join HangHoa b on a.MaNhomHang=b.MaNhomHang join ChiTietPhieuNK c on b.MaHang=c.MaHang " +
                        "join PhieuNK d on d.MaPhieuNK=c.MaPhieuNK where DATEPART(qq,(d.NgayLap)) = '" + cboQuy.Text + "'" +
                    "and TenNhomHang='" + cboNhomHang.Text + "'and DATEPART(yyyy,(d.NgayLap))='" + txtNam.Text + "'"));
                    txtTongTien.Text = tong.ToString();
                    lblBangChu.Text = "Bằng chữ: " + Class.Functions.ChuyenSoSangChu(tong.ToString());
                }
        ...
        private void btnIn_Click(object sender, EventArgs e)
        {
            ...
            exRange.Range["D2:F2"].Value = "BÁO CÁO NHẬP HÀNG";
            string sql = "Select a.MaNhomHang,b.TenHang,c.SoLuong,c.DonGia,d.NgayLap, c.TongCong from NhomHang a join HangHoa b on a.MaNhomHang=b.MaNhomHang join ChiTietPhieuNK c on b.MaHang=c.MaHang join PhieuNK d on d.MaPhieuNK=c.MaPhieuNK WHERE 1=1";
            if (cboNhomHang.Text != "")
                sql = sql + " AND a.TenNhomHang Like '%" + cboNhomHang.Text + "%' ";
            if (cboThang.Text != "")
                sql = sql + " AND MONTH(d.NgayLap) Like '%" + cboThang.Text + "%' ";
            ...
            if ((cboNhomHang.Text != "") && (cboQuy.Text != "") && (txtNam.Text != ""))
            {
                tong = Convert.ToDouble(Class.Functions.GetFieldValues("select sum(c.TongCong) from ThueMay where DATEPART(qq,(NgayThue)) = '" + cboQuy.Text + "'" +
                "and TenNhomHang='" + cboNhomHang.Text + "'and DATEPART(yyyy,(NgayThue))='" + txtNam.Text + "'"));
                exRange.Value2 = tong.ToString();
                exRange = exSheet.Cells[1][hang + 11]; //Ô A1 
                exRange.Range["A1:F1"].MergeCells = true;
                exRange.Range["A1:F1"].Font.Bold = true;
